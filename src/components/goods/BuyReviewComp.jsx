...
import GoodsPurchase from "../components/PurchaseComponent";
...
export const BuyReviewComp = () => {
  ...
  const fetchGoodsInfo = async () => {
    try {
      const response = await AxiosApi.getGoodsInfo(21);
  ...
  const addToCart = async () => {
    if (user) {
      if (isInCart) {
        try {
          const response = await AxiosApi.removeFromCart(user.id, GoodsInfo.id);
          if (response.status === 200) {
            setIsInCart(false);
        ...
        try {
          const response = await AxiosApi.addToCart(user.id, GoodsInfo.id);
          if (response.status === 200) {
            setIsInCart(true);
            alert("장바구니에 추가되었습니다.");
            navigate("/CartPage");
  ...
  const checkIfGoodsPurchased = async () => {
    try {
      if (user) {
        const response = await AxiosApi.checkPurchase(user.id, GoodsInfo.id);
        if (response.status === 200) {
  ...
  const purchaseGoods = async () => {
    if (user.cash >= GoodsInfo.price && user) {
      try {
        const response = await AxiosApi.purchaseGoods(user.id, GoodsInfo.id);
        if (response.status === 200) {
          alert("구매가 완료되었습니다.");
